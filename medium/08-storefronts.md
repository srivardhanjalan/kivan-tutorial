...
*Zero to Shipped, step 8. A wish you pick from a store instead of typing from scratch: a seeded catalog the server reads but can never overwrite, dropped into any wishlist through the same endpoint the manual form already uses. Plus the duplication that piled up every time I added a screen beside an old one.*
...
Like the life-events seed, `seed_storefronts.py` is not optional: without it `GET /storefronts` returns an empty list and the Wish Store tab has nothing to browse. It is idempotent, so a re-run is harmless. And if the image rebuild dies with a bare `CREATE_FAILED` and no logs, run `terraform apply` again: App Runner's create is intermittently flaky, Terraform taints the failed service, and a plain retry brings it up on identical config.

## What bit me

Two things cost me time on this step, worst first.

**Adding a screen re-created the one next to it.** `ProductCard` started life as a copy of `WishCard`. The product detail shared the wish detail's title, price, and blurb styles byte for byte. Both details spelled the same link-open with the same error string. And the wishlist picker hand-rolled its own confirm and cancel buttons when a `ConfirmCancelButtons` component already existed. jscpd caught the byte-for-byte clone, a semantic pass caught the ones that only matched in shape, and each fix exposed the next: extract the shared card, and the shared detail block surfaces; extract that, and the duplicated link-open is what's left. I ran the gate until a full pass found nothing. A new screen modeled on an old one is a duplication suspect before it is anything else.

**The design system only had the tokens earlier steps needed.** Porting these screens, I reached for `Spacing.xs` and `Typography.caption`, and neither exists in this codebase: it adds a token the step a component first needs it, never in advance. tsc caught both instantly. The fix was to use what is actually there, `Typography.bodySecondary` and a bare literal for the one 2px hairline that is smaller than any scale step, not to reintroduce a token this step has no second use for. A value the app "should" have is missing on purpose.
