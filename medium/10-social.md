...
Three things cost me time here, worst first. The gate and a device caught two; the first broke the front door.

**Every email-and-password signup provisioned into a 500, and only a live account showed it.** The name search index keys on `name_lowercase`. Email signup has no name, so provisioning wrote `name_lowercase = ""`, which DynamoDB rejects: an index key can't be an empty string. The record never got created, so `GET /users/me` 500ed forever; the retry `PUT` rewrote that empty string. It survived static checks and three review passes because until then every user had a name (seed, Google, Apple); no test user took the nameless path. The fix is the sparse-index idiom: omit `name_lowercase` when blank, and clear a name with `REMOVE`, not `SET` to `""`. A static gate proves the code is clean, not that it survives a real user.

**The follow count updated a beat late while the love count did it right.** I gave the follow button its own local state; Follow flipped the pill but the follower stat lagged until a refetch. The love heart moved its tally on the tap: count and control were one component. Lifting both onto one `useOptimisticToggle` fixed the lag, and retired `initialCount`'s needless option: its zero default had covered for the caller that skipped a count, until both passed one.

**The baseline copy dragged in two untrue comments.** Step 10 copies step 9 byte-for-byte, before step 9's late comment fixes. Two rode in: a seed comment naming Nike as a store with its own scraper (the bespoke scrapers are Zara, Nykaa, Puma, and Apple), and a doc citing "the add tile's plus" as an overlay the component never passes. I applied step 9's corrections verbatim: a clean copy is only as honest as its source.
