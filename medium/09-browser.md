...
- **The scraper picks the one real price, and reports the currency it found it in.** It prefers the store's own structured metadata (the live selling price, free of MRP and EMI prose), and when it has to read the page body it filters out the figures that are not the price. Whatever symbol it lands on decides the currency.
...
Most stores carry that metadata. Apple does not: its price lives in the page body with all the noise, and no clean tag to fall back on. So Apple is the one bespoke scraper, and it earns two rules the shared factory cannot express. It scans rupees only, because `apple.com/in` quotes solely in rupees. It rejects any amount under ₹10,000, which on Apple's catalog is always an instalment, never a price. And it rejects a figure that is itself a monthly instalment, checked as a tight suffix on the amount rather than through a context window.

That last rule is the subtle one, and it is worth seeing why the obvious version fails. The usual way to reject a noisy price is to look at the words around it, a window of characters on each side, and drop it if "EMI" or "trade-in" is nearby. But Apple writes the EMI and the real price right next to each other: "From ₹12199.00/mo. ... or ₹69900.00". A window wide enough to catch the "/mo." on the instalment also catches the real price sitting beside it, and throws the real one away. So the instalment check is not a window at all. It reads the twelve characters immediately after the amount and disqualifies it only if a "/mo" or "per month" unit follows it directly. The EMI disqualifies itself; the real price beside it survives.

[![apple.ts: the bespoke scraper scans rupees only, floors instalment-sized amounts, and rejects a figure only when an EMI unit follows it directly](https://raw.githubusercontent.com/srivardhanjalan/kivan-tutorial/main/mocks/mocks-09-code-apple.png?v=PLACEHOLDER)](https://github.com/srivardhanjalan/kivan-tutorial/blob/main/09-browser/frontend/src/scrapers/brands/apple.ts)
...
Every scan, bespoke or shared, goes through one currency-aware matcher. It knows the symbols the app supports, lists the multi-character dollar signs before the bare `$` so a Singapore price is never half-read as a US one, and reports the currency implied by whatever symbol it matched. That is the thread that carries the currency out of the page and into the wish.
...
A wish grows one field this step: `cost_currency`, the ISO code the price was quoted in. It is a plain string label, not a `Decimal`, so it stores and reads with no coercion. A wish captured from a browsed store carries it. A wish typed by hand or added from step 8's catalog carries none, and reads in the app's default symbol, so the single-currency look is unchanged everywhere it was before.

[![wishes.py: cost_currency, a plain ISO-code label captured beside cost, and brand_id, the browser-captured origin, both stamped once at create and left untouched by update](https://raw.githubusercontent.com/srivardhanjalan/kivan-tutorial/main/mocks/mocks-09-code-wishmodel.png?v=PLACEHOLDER)](https://github.com/srivardhanjalan/kivan-tutorial/blob/main/09-browser/backend/app/models/wishes.py)
...
![A browser-captured wish, back in the list: the AirPods wish reads "From Apple" above its title and its cost is ₹67,900, in rupees, the currency apple.com/in quoted, while the catalog wish beside it reads "From Nestwell"; both badges resolve from the origin the add-flow stamped on](https://raw.githubusercontent.com/srivardhanjalan/kivan-tutorial/main/mocks/mocks-09-fig-badge.png?v=PLACEHOLDER)
...
**The scraper kept picking a decoy price.** This is the one that bit hardest, because it passed every test I could write and then failed on the real page. I browsed to an iPhone on `apple.com/in`, tapped Add, and the wish came back priced at ₹57,000. The real price was ₹69,900. My first fix skipped small numbers to dodge the EMI figures, which is exactly how ₹57,000 got picked: it is the top of the "get ₹2500 to ₹57000 for your trade-in" credit line, a big rupee number that is not the price. I chased it into the noise filter first, widening the window of words I searched around each price for "trade-in". That made it worse, because the window that finally caught the trade-in wording also caught the real price two lines down and rejected it too. Every price on the page was now getting thrown out. The fix was two separate ideas. Read the store's structured `og:price` metadata first, where a clean selling price usually lives, so most stores never touch the noisy body at all. And for Apple, which has no such metadata, check for the "/mo." instalment unit as a tight suffix on each amount rather than through a window, so an EMI figure disqualifies only itself and not the price beside it. I verified the fix against four live pages, not one, and it caught a second bug I had not gone looking for: Nykaa was returning its struck-through MRP, ₹649, instead of the ₹422 it was actually selling at. The structured-price preference fixed that one for free, because Nykaa publishes the real ₹422 in `product:price:amount`. Apple went from ₹57,000 to ₹69,900, Nykaa from ₹649 to ₹422, and Zara and Puma did not budge.
